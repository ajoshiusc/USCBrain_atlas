...
def reduce3_to_bci_lh(reduce3labs):
    ...
    ''' reduce3 to h32k'''
    r3 = readdfs('/data_disk/HCP_data/reference.old/100307.aparc.a2009s.32k_fs.reduce3.very_smooth.left.dfs')
    r3.labels=np.squeeze(reduce3labs.T)

    '''h32k to full res FS'''
    g_surf = nib.load('/data_disk/HCP_data/reference.old/100307/MNINonLinear/Native/100307.L.very_inflated.native.surf.gii')
    h.vertices = g_surf.darrays[0].data
    h.faces = g_surf.darrays[1].data
    h = interpolate_labels(r3, h)

    ''' native FS ref to native FS BCI'''
    g_surf = nib.load('/data_disk/HCP_data/reference.old/100307/MNINonLinear/Native/100307.L.sphere.reg.native.surf.gii')
...
def reduce3_to_bci_rh(reduce3labs):
    ...
    ''' reduce3 to h32k'''
    r3 = readdfs('/data_disk/HCP_data/reference.old/100307.aparc.a2009s.32k_fs.reduce3.very_smooth.right.dfs')
    r3.labels=np.squeeze(reduce3labs.T)

    '''h32k to full res FS'''
    g_surf = nib.load('/data_disk/HCP_data/reference.old/100307/MNINonLinear/Native/100307.R.very_inflated.native.surf.gii')
    h.vertices = g_surf.darrays[0].data
    h.faces = g_surf.darrays[1].data
    h = interpolate_labels(r3, h)

    ''' native FS ref to native FS BCI'''
    g_surf = nib.load('/data_disk/HCP_data/reference.old/100307/MNINonLinear/Native/100307.R.sphere.reg.native.surf.gii')
    s.vertices = g_surf.darrays[0].data
    s.faces = g_surf.darrays[1].data
    s.labels = h.labels
