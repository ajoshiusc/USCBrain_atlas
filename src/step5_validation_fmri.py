...
def parcellate_region(roilist, sub, nClusters, scan, scan_type, savepng=0, session=1, algo=0, type_cor=0):
    p_dir = '/data_disk/HCP100-fMRI-NLM/HCP100-fMRI-NLM' # '/big_disk/ajoshi/HCP100-fMRI-NLM/HCP100-fMRI-NLM'
    r_factor = 3
    ...
    dfs_left_sm = readdfs(
        os.path.join('/ImagePTE1/ajoshi/code_farm/USCBrain_atlas/data', '100307.BCI2reduce3.very_smooth.' + scan_type + '.dfs'))
    dfs_left = readdfs(os.path.join('/ImagePTE1/ajoshi/code_farm/USCBrain_atlas/data', '100307.BCI2reduce3.very_smooth.' + scan_type + '.dfs'))
...
p_dir = '/data_disk/HCP100-fMRI-NLM/HCP100-fMRI-NLM'
lst = os.listdir(p_dir) #{'100307'}
old_lst = os.listdir('/data_disk/HCP_data/data')
old_lst+=['reference','zip1']
save_dir= '/ImagePTE1/ajoshi/code_farm/USCBrain_atlas/validation'
...
for sub in lst:
    for scan in range(0,4):
        if (sub not in old_lst) and (os.path.isfile(os.path.join(p_dir, sub, sub + fadd_1 + str(session_type[scan%2]) + sdir[int(np.floor(scan/2))] + fadd_2))):
            print(sub)
            for i in range(0,2):
                ...
                for n in range(nClusters.shape[0]):
                    ...
                    labs1, correlation_within_roi_vector, correlation_with_rest_vector, mask, centroid = parcellate_region(
                        roiregion, sub, nClusters[n], sdir[int(np.floor(scan/2))], scan_type[i],
                ...
                sp.savez(os.path.join(save_dir, str(sub) + '_' + scan_type[i]  + sdir[int(np.floor(scan/2))] + '_' + str(session_type[scan%2]) + '.npz'),
