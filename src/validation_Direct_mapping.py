...
e = xml.etree.ElementTree.parse('/home/ajoshi/BrainSuite19b/svreg/BCI-DNI_brain_atlas/brainsuite_labeldescription.xml').getroot()
for hemi in range(0,2):
    ...
    for i in range(T1.__len__()):
        flag = 0
        for j in range(5):
            if (T1[i] * 10 + j + 1) in refined_roilists.keys():
